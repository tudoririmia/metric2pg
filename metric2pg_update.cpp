...
int metric2pg_update (std::list <char*> arguments) {
    ...
    try {
      ...
      string sql = "INSERT INTO " + std::string(table) + "(time, temperature)" \
                   "VALUES(NOW()," + std::string(value) + ");";
      ...
      W.exec( sql );
      W.commit();
      cout << "Table updated successfully with value: " << value << endl;
      C.disconnect ();
